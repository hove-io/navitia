...
| nop     | max_duration         | int     | Maximum duration to the beginning of the journey public transports section in seconds (from `datetime` parameter).<br>If `datetime` represents the arrival of the journeys requested, then the journey public transports section must start after `datetime` - `max_duration` <br>More useful when computing an isochrone (only `from` or `to` is provided)    | 86400       |
