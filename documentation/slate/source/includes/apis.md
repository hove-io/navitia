...
| nop     | direct_path          | enum    | Specify if direct paths (fallback modes only are used) should be suggested.<br>possible values: <ul><li>`indifferent`</li><li>`none` to have only journeys using some PT</li><li>`only` to have only journeys without PT</li></ul>      | indifferent |
