...
| `get` /coverage/*lon;lat*/stop_schedules        | List of the stop schedules grouped by ``stop_point/route`` for coordinates                                    |
...
| `get` /coverage/*lon;lat*/departures            | List of the next departures, multi-route oriented, only time sorted (no grouped by ``stop_point/route`` here) |
...
| `get` /coverage/*lon;lat*/arrivals              | List of the arrivals, multi-route oriented, only time sorted (no grouped by ``stop_point/route`` here)        |
