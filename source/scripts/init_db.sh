...
sudo -u postgres psql --set ON_ERROR_STOP=1 --dbname=$dbname -f "$postgis_dir/postgis.sql"

sudo -u postgres psql --set ON_ERROR_STOP=1 --dbname=$dbname -f "$postgis_dir/spatial_ref_sys.sql"
