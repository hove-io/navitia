...
*/
    way.name="AA";
    way.way_type="rue";
    nt::GeographicalCoord upper(1.0,53.0, false);
    nt::GeographicalCoord lower(1.0,3.0, false);
...
// Numéro recherché est = à numéro dans la rue
    ...
    BOOST_CHECK_EQUAL(result, nt::GeographicalCoord(1.0,17.0, false));
...
// Numéro recherché est n'existe pas mais il est inclus entre le plus petit et grand numéro de la rue
   ...
   BOOST_CHECK_EQUAL(result, nt::GeographicalCoord(1.0,43.0, false));

// liste des numéros pair est vide ==> Calcul du barycentre de la rue
   result = way.nearest_coord(40, graph);
   BOOST_CHECK_EQUAL(result, nt::GeographicalCoord(12.0/12,208.0/12.0, false));
...
*/
    way.name="AA";
    way.way_type="rue";
    nt::GeographicalCoord upper(2.0,54.0, false);
    nt::GeographicalCoord lower(2.0,4.0, false);
...
// Numéro recherché est = à numéro dans la rue
    ...
    BOOST_CHECK_EQUAL(result, nt::GeographicalCoord(2.0,18.0, false));

// Numéro recherché est n'existe pas mais il est inclus entre le plus petit et grand numéro de la rue
    // ==>  Extrapolation des coordonnées
   result = way.nearest_coord(44, graph);
   BOOST_CHECK_EQUAL(result, nt::GeographicalCoord(2.0,44.0, false));

// liste des numéros impair est vide ==> Calcul du barycentre de la rue
   result = way.nearest_coord(41, graph);
   BOOST_CHECK_EQUAL(result, nt::GeographicalCoord(24.0/12,220.0/12.0, false));
...
// les coordonnées sont à l'extérieur de la rue coté supérieur
    int result = way.nearest_number(nt::GeographicalCoord(1.0,55.0, false));
    BOOST_CHECK_EQUAL(result, 54);

// les coordonnées sont à l'extérieur de la rue coté inférieur
    result = way.nearest_number(nt::GeographicalCoord(2.0,3.0, false));
    BOOST_CHECK_EQUAL(result, 4);

// coordonnées recherchées est = à coordonnées dans la rue
    result = way.nearest_number(nt::GeographicalCoord(2.0,8.0, false));
