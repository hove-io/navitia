...
BOOST_AUTO_TEST_CASE(create_vj_test) {
    ...
    vp.days = year(
        "0000100"
        "0000100");
    const auto* adapted_vj =
        mvj->create_discrete_vj("vj:adapted", "adapted", nt::RTLevel::Adapted, vp, base_vj->route, sts, pt_data);
    ...
    vp.days = year(
        "0000000"
        "0000110");
    const auto* rt_vj = mvj->create_discrete_vj("vj:rt", "rt", nt::RTLevel::RealTime, vp, base_vj->route, sts, pt_data);
