...
pbnavitia::Response traffic_reports(const navitia::type::Data& d,
    ...
    for (const NetworkDisrupt& dist: disrupts) {
        ...
        for(const auto& impact: dist.network_disruptions){
            pb_creator.fill(depth-1, DumpMessage::Yes, impact.get(), pb_network);
        }
        pb_creator.fill(depth, DumpMessage::No, dist.network, pb_network);
        for (const auto& line_item: dist.lines) {
            pbnavitia::Line* pb_line = pb_traffic_reports->add_lines();
            pb_creator.fill(depth-1, DumpMessage::No, line_item.first, pb_line);
            for(const auto& impact: line_item.second){
                pb_creator.fill(depth-1, DumpMessage::Yes, impact.get(), pb_line);
            }
        }
        for (const auto& sa_item: dist.stop_areas) {
            pbnavitia::StopArea* pb_stop_area = pb_traffic_reports->add_stop_areas();
            pb_creator.fill(depth-1, DumpMessage::No, sa_item.first, pb_stop_area);
            for(const auto& impact: sa_item.second){
                pb_creator.fill(depth-1, DumpMessage::Yes, impact.get(), pb_stop_area);
            }
        }
        for (const auto& vj_item: dist.vehicle_journeys) {
            ...
            pb_creator.fill(depth-1, DumpMessage::No, vj_item.first, pb_vehicle_journey);
            for(const auto& impact: vj_item.second){
                pb_creator.fill(depth-1, DumpMessage::Yes, impact.get(), pb_vehicle_journey);
            }
        }
    }
