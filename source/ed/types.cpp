...
bool Poi::init_osm(const uint64_t osm_id, const double lon, const double lat,
            const std::map<std::string, std::string>& tags, const uint64_t potential_id,
        ...
        this->poi_type = it->second;
        if(tags.find("name") != tags.end()){
            this->name = tags.at("name");
        }
        for(auto st : tags){
            const auto it_property = std::find(properties_to_ignore.begin(),
                    properties_to_ignore.end(), st.first);
            if(it_property == properties_to_ignore.end()) {
                if(st.first == "addr:housenumber"){
                    this->address_number = st.second;
                }
                if(st.first == "addr:street"){
                    this->address_name = st.second;
                }
            }else{
                this->properties[st.first] = st.second;
            }
        }
        this->id = potential_id;
        this->osm_id = osm_id;
        this->coord.set_lon(lon);
        this->coord.set_lat(lat);
        return true;
}
