...
void FileParser::read() {
    csv_row headers = required_headers();
...
void AgencyGtfsParser::init() {
    id_c = csv.get_pos_col("agency_id");
    name_c = csv.get_pos_col("agency_name");
}

void AgencyGtfsParser::handle_line(const csv_row& row, bool is_first_line) {
...
void ContributorGtfsParser::init() {
    id_c = csv.get_pos_col("agency_id");
    name_c = csv.get_pos_col("agency_name");
}

void ContributorGtfsParser::handle_line(const csv_row& row, bool is_first_line) {
    if(! is_first_line && id_c == -1) {
        LOG4CPLUS_FATAL(logger, "Error while reading " + csv.filename +
...
void CompanyGtfsParser::handle_line(const csv_row& row, bool is_first_line) {
    if(! is_first_line && id_c == -1) {
        LOG4CPLUS_FATAL(logger, "Error while reading " + csv.filename +
...
void PhysicalModeGtfsParser::init() {
    id_c = csv.get_pos_col("physical_mode_id");
    name_c = csv.get_pos_col("physical_mode_name");
}

void PhysicalModeGtfsParser::handle_line(const csv_row& row, bool is_first_line) {
    if(! is_first_line && id_c == -1) {
        LOG4CPLUS_FATAL(logger, "Error while reading " + csv.filename +
...
void CommercialModeGtfsParser::init() {
    id_c = csv.get_pos_col("commercial_mode_id");
    name_c = csv.get_pos_col("commercial_mode_name");
}

void CommercialModeGtfsParser::handle_line(const csv_row& row, bool is_first_line) {
    if(! is_first_line && id_c == -1) {
        LOG4CPLUS_FATAL(logger, "Error while reading " + csv.filename +
...
template <typename T>
bool StopsGtfsParser::parse_common_data(const csv_row& row, T* stop) {
    try{
...
void StopsGtfsParser::handle_line(const csv_row& row, bool) {
    if(row.empty())
        return;
...
void RouteGtfsParser::handle_line(const csv_row& row, bool) {
    if(row.empty())
        return;
...
void CalendarGtfsParser::finish() {
...
void CalendarGtfsParser::handle_line(const csv_row& row, bool) {
    if (row.empty())
        return;
    std::bitset<7> week;
    nblignes ++;
...
void CalendarDatesGtfsParser::handle_line(const csv_row& row, bool) {
    if(row.empty())
        return;
...
void TripsGtfsParser::handle_line(const csv_row& row, bool) {
    if(row.empty())
        return;
...
//void ContributorGtfsParser::handle_line(const csv_row& row, bool is_first_line) {
//}
GtfsParser::GtfsParser(const std::string & path) : path(path) {
    init_logger();
    logger = log4cplus::Logger::getInstance("log");
}


const std::string append(const std::string& path, const std::string name) {
    return path + "/" + name;
}
...
void GtfsParser::fill(Data & data, const std::string beginning_date) {
    ...
    parsers.push_back(std::make_unique<AgencyGtfsParser>(this->gtfs_data, data, append(path, "agency.txt")));
    parsers.push_back(std::make_unique<ContributorGtfsParser>(this->gtfs_data, data, append(path, "contributor.txt")));
    parsers.push_back(std::make_unique<CompanyGtfsParser>(this->gtfs_data, data, append(path, "company.txt")));
    parsers.push_back(std::make_unique<PhysicalModeGtfsParser>(this->gtfs_data, data, append(path, "physical_mode.txt")));
    parsers.push_back(std::make_unique<CommercialModeGtfsParser>(this->gtfs_data, data, append(path, "commercial_mode.txt")));
    parsers.push_back(std::make_unique<StopsGtfsParser>(this->gtfs_data, data, append(path, "stops.txt")));
    parsers.push_back(std::make_unique<RouteGtfsParser>(this->gtfs_data, data, append(path, "routes.txt")));
    parsers.push_back(std::make_unique<TransfersGtfsParser>(this->gtfs_data, data, append(path, "transfers.txt")));
    parsers.push_back(std::make_unique<CalendarGtfsParser>(this->gtfs_data, data, append(path, "calendar.txt")));
    parsers.push_back(std::make_unique<CalendarDatesGtfsParser>(this->gtfs_data, data, append(path, "calendar_dates.txt")));
    parsers.push_back(std::make_unique<TripsGtfsParser>(this->gtfs_data, data, append(path, "trips.txt")));
    ...
    for(auto& parser : parsers) {
        LOG4CPLUS_TRACE(logger, "Parsing : " + parser->csv.filename);
        if (!parser->csv.is_open()) {
            LOG4CPLUS_FATAL(logger, "Impossible to read " + parser->csv.filename);
            if (required_files.find(parser->csv.filename) != required_files.end()) {
                throw FileNotFoundException(parser->csv.filename);
            }
            continue;
        }
        parser->read();
...
void GtfsParser::fill_default_objects(Data & data){
    ...
    for(ed::types::CommercialMode *mt : data.commercial_modes) {
        ...
        mode->uri = mt->uri;
        data.physical_modes.push_back(mode);
        gtfs_data.physical_mode_map[mode->id] = mode;
    }
