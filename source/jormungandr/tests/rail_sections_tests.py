...
class TestRailSections(AbstractTestFixture):
    ...
    def test_disruption_api_with_rail_section(self):
        ...
        assert len(get_not_null(r, 'disruptions')) == 10
    ...
    def test_journeys_with_rail_section(self):
        ...
        # All the stops from stopD1 to stopI1 are impacted
        ...
        # This test relates to https://navitia.atlassian.net/browse/NAV-1820
        # Test on line 'line:100' impacted with a rail_section with severity=OTHER_EFFECT
        # Impacted from D1 to B1 with blocked stop C1
        # journeys from stopI1-> stopE1 / Base_schedule: No disruption
        r = journeys(_from='stopI1', to='stopE1')
        assert len(r["journeys"]) == 1
        assert get_used_vj(r) == [['vehicle_journey:vj:100-2']]
        d = get_all_element_disruptions(r['journeys'], r)
        assert not d

        # Test on line 'line:101' impacted with two rail_sections and severity=NO_SERVICE
        # Stops: P1, Q1, R1, S1, T1, U1, V1, W1
        # Impacted from S1 to V1 with blocked stops T1 and U1
        # Impacted from V1 to S1 with blocked stops U1 and T1
        # All the stops from stopT1 toward stopW1 are impacted by forward rail_section
        # All the stops from stopU1 toward stopP1 are impacted by backward rail_section
        ...
        # stopR1 -> stopU1 / Base_schedule: A disruption to display on vj:101-1
        scenario = {
            'rail_section_on_line101': True,
        }

        r = journeys(_from='stopR1', to='stopU1')
        assert len(r["journeys"]) == 1
        assert get_used_vj(r) == [['vehicle_journey:vj:101-1']]
        d = get_all_element_disruptions(r['journeys'], r)
        assert impacted_headsigns(d) == {'vj:101-1'}
        ...
        # stopV1-> stopW1 / Base_schedule: A disruption to display on vj:101-1
        scenario = {
            'rail_section_on_line101': True,
        }

        r = journeys(_from='stopV1', to='stopW1')
        assert len(r["journeys"]) == 1
        assert get_used_vj(r) == [['vehicle_journey:vj:101-1']]
        d = get_all_element_disruptions(r['journeys'], r)
        assert impacted_headsigns(d) == {'vj:101-1'}
        ...
        # stopW1-> stopV1 / Base_schedule: No disruption
        scenario = {
            'rail_section_on_line101': False,
        }

        r = journeys(_from='stopW1', to='stopV1')
        assert len(r["journeys"]) == 1
        assert get_used_vj(r) == [['vehicle_journey:vj:101-2']]
        d = get_all_element_disruptions(r['journeys'], r)
        assert not impacted_headsigns(d)
    ...
    def test_traffic_reports_on_networks(self):
        ...
        assert len(r['traffic_reports'][0]['stop_areas']) == 43
    ...
    def test_stops_schedules_and_departures_impacted_by_rail_section(self):
        ...
        for q in ['departures', 'arrivals', 'stop_schedules']:
            ...
            scenario = {
                'rail_section_on_line100': False,
                'rail_section_on_line100_route100_route-2': True,
            }
            ...
            for disruption, result in scenario.items():
                assert result == (disruption in d)
            if q in ['departures', 'arrivals']:
                assert impacted_headsigns(d) == {'vj:100-2'}
            if q == 'stop_schedules':
                assert impacted_ids(d) == {'vehicle_journey:vj:100-2'}
