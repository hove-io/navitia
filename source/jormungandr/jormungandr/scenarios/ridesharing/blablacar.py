...
class Blablacar(AbstractRidesharingService):
    ...
    def _call_service(self, params):
        self.logger.debug("requesting blablacar")

        try:
            return self.breaker.call(
                requests.get, url=self.service_url, headers={}, params=params, timeout=self.timeout
            )
        except pybreaker.CircuitBreakerError as e:
            logging.getLogger(__name__).error(
                'Blablacar service dead (error: %s)', e, extra={'ridesharing_service_id': self._get_rs_id()}
            )
            raise RidesharingServiceError('circuit breaker open')
        except requests.Timeout as t:
            logging.getLogger(__name__).error(
                'Blablacar service timeout (error: %s)', t, extra={'ridesharing_service_id': self._get_rs_id()}
            )
            raise RidesharingServiceError('timeout')
        except Exception as e:
            logging.getLogger(__name__).exception(
                'Blablacar service error', extra={'ridesharing_service_id': self._get_rs_id()}
            )
