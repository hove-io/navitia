...
from jormungandr.scenarios.helpers import (
    select_best_journey_by_time,
    select_best_journey_by_duration,
...
class Scenario(simple.Scenario):
    ...
    def call_kraken(self, req, instance, tag=None):
        ...
        for o_mode, d_mode in itertools.product(self.origin_modes, self.destination_modes):
            ...
            if o_mode == 'car' or (
                is_admin(req.journeys.origin[0].place) and is_admin(req.journeys.destination[0].place)
            ):
        ...
        def get_uri_odt_with_zones(journey):
            ...
            for section in journey.sections:
                if section.type == response_pb2.PUBLIC_TRANSPORT:
                    ...
                    if response_pb2.ODT_WITH_ZONE in tags or response_pb2.ODT_WITH_STOP_POINT in tags:
    ...
    def fill_journeys(self, pb_req, request, instance):
        ...
        while (
            (request["min_nb_journeys"] and request["min_nb_journeys"] > nb_typed_journeys)
            or (not request["min_nb_journeys"] and nb_typed_journeys == 0)
        ) and cpt_attempt < max_attempts:
    ...
    def next_journey_datetime(journeys, clockwise):
        ...
        if not journeys:
            return None
        last_best = next((j for j in journeys if j.type in ('best', 'rapid')), None) or min(
            journeys, key=lambda j: j.departure_date_time
        )
    ...
    def previous_journey_datetime(journeys, clockwise):
        ...
        if not journeys:
            return None
        last_best = next((j for j in journeys if j.type in ('best', 'rapid')), None) or max(
            journeys, key=lambda j: j.arrival_date_time
        )
    ...
    def _find_max_duration(self, journeys, instance, clockwise):
        ...
        if not asap_journey:
            return None
        return (
            asap_journey.duration * instance.factor_too_long_journey
        ) + instance.min_duration_too_long_journey
    ...
    def merge_response(self, initial_response, new_response):
        ...
        initial_response.feed_publishers.extend(
            [fp for fp in new_response.feed_publishers if fp.id not in initial_feed_publishers]
        )
    ...
    def delete_journeys(self, resp, request, final_filter):
        ...
        if "clockwise" not in request or request['clockwise']:
            if (
                ...
                and isinstance(request['destination'], list)
        ...
        if "clockwise" in request and not request['clockwise']:
            if "origin" in request and isinstance(request['origin'], list) and len(request['origin']) > 1:
                return  # for n-m calculation we don't want to filter
    ...
    def _remove_not_long_enough_fallback(self, resp, instance):
        ...
        logger = logging.getLogger(__name__)
        logger.debug(
            'remove %s journey with not enough fallback duration: %s',
            len(to_delete),
            [resp.journeys[i].type for i in to_delete],
        )
        self.erase_journeys(resp, to_delete)
    ...
    def _remove_not_long_enough_tc_with_fallback(self, resp, instance):
        ...
        logger = logging.getLogger(__name__)
        logger.debug(
            'remove %s journey with not enough tc duration: %s',
            len(to_delete),
            [resp.journeys[i].type for i in to_delete],
        )
        self.erase_journeys(resp, to_delete)
