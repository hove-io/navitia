...
class StopPointSerializer(PbGenericSerializer):
    ...
    platform_code = jsonschema.MethodField(schema_type=str, display_none=False)

    def get_platform_code(self, obj):
        return obj.platform_code if obj.HasField(str('platform_code')) else None
...
class RouteDisplayInformationSerializer(PbNestedSerializer):
    ...
    company = jsonschema.Field(schema_type=str, display_none=True)
...
class VJDisplayInformationSerializer(PassageDisplayInformationSerializer):
    def get_links(self, obj):
        response = DisruptionLinkSerializer().to_value(obj.impact_uris)
