...
def upgrade():
    ...
    op.execute("UPDATE billing_plan SET created_at = DATE(NOW()) - 365 * 5")
    op.execute("UPDATE key SET created_at = DATE(NOW()) - 365 * 5")
    op.execute("UPDATE \"user\" SET created_at = DATE(NOW()) - 365 * 5")
