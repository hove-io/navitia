""" Add attributes lockable and notify_threshold_list in the table billing_plan
...
def upgrade():
    op.add_column('billing_plan', sa.Column('lockable', sa.Boolean(), nullable=False, server_default="false"))
    op.add_column(
        'billing_plan',
        sa.Column('notify_threshold_list', postgresql.ARRAY(sa.Integer()), server_default='{}', nullable=True),
    )
    # Fill attributes lockable and notify_threshold_list depending on max_request_count and end_point
    op.execute(
    ...
    op.execute(
        "update billing_plan set lockable=true where max_request_count > 0 "
...
def downgrade():
    ...
    op.drop_column('billing_plan', 'lockable')
