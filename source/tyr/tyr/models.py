...
class User(db.Model):
    ...
    def add_key(self, valid_until=None):
        """
        génére une nouvelle clé pour l'utilisateur et l'ajoute à sa liste de clé
...
class Api(db.Model):
    ...
    authorizations = db.relationship('Authorization', backref='api', lazy='dynamic')
...
class Authorization(db.Model):
    __table_args__ = {"schema": "jormungandr"}
    user_id = db.Column(db.Integer, db.ForeignKey('jormungandr.user.id'), primary_key=True)
    instance_id = db.Column(db.Integer, db.ForeignKey('jormungandr.instance.id'), primary_key=True)
    api_id = db.Column(db.Integer, db.ForeignKey('jormungandr.api.id'), primary_key=True)
