...
route_schedule(const std::string& filter,
    ...
    for (const auto& route_idx: routes_idx) {
        ...
        ProtoCreator::fill_pb_object(route, d, m_pt_display_informations, 0, now, action_period);
        ...
        for(unsigned int i=0; i < thermometer.get_thermometer().size(); ++i) {
            ...
            ProtoCreator::fill_pb_object(sp, d, row->mutable_stop_point(), max_depth,
            ...
            for(unsigned int j=0; j<stop_times.size(); ++j) {
                ...
                if (!is_vj_set[j] && dt_stop_time.second != nullptr) {
                    ...
                    ProtoCreator::fill_pb_object(&vj_st, d, vj_display_information, 0, now, action_period);
                ...
                const auto& st_calandar =  ProtoCreator::StopTimeCalandar(dt_stop_time.second,
                                                                          dt_stop_time.first, calendar_id);
                ProtoCreator::fill_pb_object(&st_calandar, d, pb_dt, max_depth, now, action_period);
            }
        }
        ProtoCreator::fill_pb_object(&route->shape, d, schedule->mutable_geojson());
    }
