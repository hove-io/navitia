...
std::vector<datetime_stop_time> get_stop_times(const std::vector<type::idx_t> &journey_pattern_points, const type::DateTime &dt, const type::DateTime &max_dt,
    ...
    while(test_add && result.size() < max_departures) {
        ...
        for(auto jpp_idx : journey_pattern_points) {
            ...
            if(st != nullptr) {
                type::DateTime dt_temp = next_requested_datetime[jpp_idx];
