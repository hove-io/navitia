...
def downgrade():
    op.add_column('vehicle_journey', sa.Column('journey_pattern_id', sa.BIGINT(), autoincrement=False, nullable=False), schema='navitia')
    op.drop_column('vehicle_journey', 'route_id', schema='navitia')
    op.drop_column('vehicle_journey', 'physical_mode_id', schema='navitia')
    op.add_column('stop_time', sa.Column('journey_pattern_point_id', sa.BIGINT(), autoincrement=False, nullable=False), schema='navitia')
    ...
    sa.Column('shape_from_prev', Geography(geometry_type='LINESTRING', srid=4326, spatial_index=False), primary_key=False),
