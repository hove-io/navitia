...
makePathes(std::vector<std::pair<type::idx_t, boost::posix_time::time_duration> > destinations,
           DateTime dt,
           const type::AccessibiliteParams & accessibilite_params, const RAPTOR &raptor_, bool clockwise, bool without_disrupt) {
    ...
    for(unsigned int i=1;i<=raptor_.count;++i) {
        ...
        if(best_jpp != type::invalid_idx)
            result.push_back(makePath(best_jpp, i, clockwise, without_disrupt, accessibilite_params, raptor_));
    }
...
get_current_stidx_gap(size_t count, type::idx_t journey_pattern_point, const std::vector<label_vector_t> &labels,
                      const type::AccessibiliteParams & accessibilite_params, bool clockwise,  const navitia::type::Data &data, bool without_disrupt) {
    ...
    if(label.type == boarding_type::vj) {
        ...
        return best_stop_time(jpp, label.dt, accessibilite_params.vehicle_properties, clockwise, without_disrupt, data, true);
    }
...
Path
makePath(type::idx_t destination_idx, unsigned int countb, bool clockwise, bool without_disrupt,
        ...
        if(raptor_.get_type(countb, current_jpp_idx) == boarding_type::connection ||
           ...
           raptor_.get_type(countb, current_jpp_idx) == boarding_type::connection_guarantee) {
            ...
            if(boarding_jpp == type::invalid_idx) {
                ...
                std::tie(current_st, workingDate) = get_current_stidx_gap(countb, current_jpp_idx, raptor_.labels, accessibilite_params, clockwise,  raptor_.data, without_disrupt) ;
                item = PathItem();
...
makePathreverse(unsigned int destination_idx, unsigned int countb,
    ...
    return makePath(destination_idx, countb, without_disrupt, false, accessibilite_params, raptor_);
}
