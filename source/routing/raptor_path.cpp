           ...
           raptor_.get_type(countb, current_jpp_idx) == boarding_type::connection_guarantee) {
            ...
            if(boarding_jpp == type::invalid_idx) {
                ...
                /*if(current_st->is_frequency())
                    DateTimeUtils::update(workingDate, clockwise?current_st->f_departure_time(workingDate):current_st->f_arrival_time(workingDate), clockwise);
                else
