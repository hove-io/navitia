...
std::pair<type::idx_t, uint32_t> earliest_trip(const type::Route & route, const unsigned int order, const DateTime &dt, const type::Data &data, const bool wheelchair = false);
///Cherche le premier trip partant avant dt sur la route au route point order
std::pair<type::idx_t, uint32_t> tardiest_trip(const type::Route & route, const unsigned int order, const DateTime &dt, const type::Data &data, const bool wheelchair = false);
