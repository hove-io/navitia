...
BOOST_AUTO_TEST_CASE(direct){
    ...
    auto res = raptor.compute_reverse(d.stop_areas[0].idx, d.stop_areas[1].idx, 8200, 0);
...
BOOST_AUTO_TEST_CASE(change){
    ...
    auto res = raptor.compute_reverse(d.stop_areas[0].idx, d.stop_areas[4].idx, 8500, 0);
...
BOOST_AUTO_TEST_CASE(passe_minuit){
    ...
    auto res = raptor.compute_reverse(d.stop_areas[0].idx, d.stop_areas[2].idx, 30*60, 1);
...
BOOST_AUTO_TEST_CASE(validity_pattern){
    ...
    auto res = raptor.compute_reverse(d.stop_areas[0].idx, d.stop_areas[1].idx, 9300, 0);
    BOOST_REQUIRE_EQUAL(res.items.size(), 2);
    BOOST_CHECK_EQUAL(res.items[1].arrival.hour(), 9200);

    res = raptor.compute_reverse(d.stop_areas[0].idx, d.stop_areas[1].idx, 9300, 2);
...
BOOST_AUTO_TEST_CASE(marche_a_pied_milieu){
    ...
    auto res = raptor.compute_reverse(d.stop_areas[0].idx, d.stop_areas[3].idx, 9300, 0);
...
BOOST_AUTO_TEST_CASE(marche_a_pied_pam){
    ...
    auto res = raptor.compute_reverse(d.stop_areas.at(0).idx, d.stop_areas.at(3).idx, 30, 1);
