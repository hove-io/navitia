    ...
    for(; idx < end; ++idx) {
        ...
        if (st->departure_validity_pattern->check(date)) {
            if( st->valid_end(reconstructing_path) && st->valid_hour(hour, true)
                    && st->vehicle_journey->accessible(required_properties) ){
                return st;
            }
        }
    }
...
earliest_stop_time(const type::JourneyPatternPoint* jpp,
    ...
    if(!jpp->stop_point->accessible(required_properties))
    ...
    const type::StopTime* first_st = valid_pick_up(idx, end_idx, dt.date(), dt.hour(), data, reconstructing_path, required_properties);
    ...
    if(first_st != nullptr){
        return std::make_pair(first_st, !first_st->is_frequency() ? 0 : compute_gap(dt.hour(), first_st->start_time, first_st->headway_secs));
    }
...
tardiest_stop_time(const type::JourneyPatternPoint* jpp,
    ...
    const type::StopTime* first_st = valid_drop_off(idx, end_idx, dt.date(), dt.hour(), data, reconstructing_path, required_properties);
