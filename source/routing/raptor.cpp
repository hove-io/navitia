    ...
    for(unsigned int i=1;i<=count;++i) {
        ...
        for(auto spid_dist : destinations) {
            for(auto dest : data.pt_data.stop_points[spid_dist.first].route_point_list) {
                if(labels[i][dest].type != uninitialized) {
                    DateTime current_dt = labels[i][dest].departure + (spid_dist.second/walking_speed);
...
Path RAPTOR::makePath(type::idx_t destination_idx, unsigned int countb, bool reverse) {
    ...
    label l = labels[countb][current_rpid];
    ...
    while(!stop) {
        ...
        if(labels[countb][current_rpid].type == connection || labels[countb][current_rpid].type == connection_extension
                ||  labels[countb][current_rpid].type == connection_guarantee) {
            l = labels[countb][current_rpid];
            auto r2 = labels[countb][l.rpid_embarquement];
            if(reverse) {
            ...
            item.stop_points.push_back(data.pt_data.route_points[l.rpid_embarquement].stop_point_idx);
            ...
            if(rpid_embarquement == navitia::type::invalid_idx) {
                ...
                current_st = data.pt_data.stop_times.at(l.stop_time_idx);
                uint32_t gap = l.arrival.hour() - current_st.arrival_time%data.dataRaptor.SECONDS_PER_DAY;
        ...
        if(labels[countb][current_rpid].type == depart)
            stop = true;
