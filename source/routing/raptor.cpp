...
bool RAPTOR::apply_vj_extension(const Visitor& v,
                                ...
                                DateTime base_dt,
                                DateTime working_fallback_duration,
...
std::vector<StartingPointSndPhase> make_starting_points_snd_phase(const RAPTOR& raptor,
    ...
    for (unsigned count = 1; count <= raptor.count; ++count) {
        ...
        for (const auto& a : arrs) {
            ...
            const DateTime fallback_duration_to_departure_stop_point = working_labels.fallback_duration_pt(a.first);
            const DateTime total_fallback_duration =
                fallback_duration_to_arrival_stop_point + fallback_duration_to_departure_stop_point;
            ...
            StartingPointSndPhase starting_point = {a.first, count, arrival_date_time, total_fallback_duration, false};
...
RAPTOR::Journeys RAPTOR::compute_all_journeys(const map_stop_point_duration& departures,
    ...
    LOG4CPLUS_DEBUG(logger, "Before second pass, nb of solutions : " << solutions.size());
    ...
    // return raw results
    LOG4CPLUS_DEBUG(logger, "Solutions in pool :");
    for (auto& journey : solutions.get_pool()) {
        LOG4CPLUS_DEBUG(logger, "" << journey);
    }
    ...
    while (continue_algorithm && count <= max_transfers) {
        ...
        for (auto q_elt : Q) {
            ...
            if (q_elt.second != visitor.init_queue_item()) {
                ...
                DateTime working_fallback_duration = DateTimeUtils::not_valid;
                ...
                for (const dataRAPTOR::JppsFromJp::Jpp& jpp : jpps_to_explore) {
                    ...
                    const DateTime previous_fallback_duration = prec_labels.fallback_duration_transfer(jpp.sp_idx);
                    ...
                    bool update_boarding_stop_point =
                        ...
                        || (tmp_st_dt.second == workingDt && previous_fallback_duration <= working_fallback_duration);
                    ...
                    if (update_boarding_stop_point) {
                        ...
                        if (tmp_st_dt.first != nullptr) {
                            ...
                            // } else {
                            //     LOG4CPLUS_TRACE(logger, "Switching boarding stop point : "
                            //                                 << " from "
                            //                                 <<
                            //                                 data.pt_data->stop_points[boarding_stop_point.val]->uri
                            //                                 << " to " <<
                            ...
                            working_fallback_duration = previous_fallback_duration;
                ...
                if (is_onboard) {
                    ...
                    if (vj_stay_in) {
                        bool applied = apply_vj_extension(visitor, rt_level, vj_stay_in, l_zone, base_dt,
                                                          working_fallback_duration, boarding_stop_point);
...
std::string RAPTOR::print_best_labels() {
    ...
    for (uint32_t stop_point_id = 0; stop_point_id < data.pt_data->stop_points.size(); ++stop_point_id) {
        ...
        output << "" << stop_point->uri << " : "
               << "best pt : " << iso_string(best_labels_pts[sp_idx], data)
               << " best transfer : " << iso_string(best_labels_transfers[sp_idx], data) << std::endl;
    }
    return output.str();
}
...
std::string RAPTOR::print_starting_points_snd_phase(std::vector<StartingPointSndPhase>& starting_points) {
    ...
    for (auto start_point : starting_points) {
        ...
        output << "" << stop_point->uri << " count : " << start_point.count
               ...
               << " fallback_dur : " << navitia::str(start_point.fallback_dur)
